...
class UnitedAirlinesEnhancedPredictor:
    ...
    def predict(self, flight_data):
        """
        Make predictions on new flight data
        
        Args:
            flight_data (dict): Dictionary containing flight information
                Required keys: Origin, Dest, CRSDepTime, CRSArrTime, FlightDate
                Optional keys: temperature_c, precip_mm, cloud_pct, wind_speed_mps
        
        Returns:
            dict: Prediction results with delay estimate and confidence
        """
        if self.best_model is None:
        ...
        try:
            ...
            required_cols = ['Origin', 'Dest', 'CRSDepTime', 'CRSArrTime', 'FlightDate']
            for col in required_cols:
                if col not in input_df.columns:
                    raise ValueError(f"Missing required column: {col}")
            
            # Convert FlightDate to datetime
            input_df['FlightDate'] = pd.to_datetime(input_df['FlightDate'])
            
            # Add derived features
            input_df['DayOfWeek'] = input_df['FlightDate'].dt.dayofweek
            input_df['Month'] = input_df['FlightDate'].dt.month
            input_df['Quarter'] = input_df['FlightDate'].dt.quarter
            
            # Convert time to minutes since midnight
            input_df['CRSDepTime'] = pd.to_numeric(input_df['CRSDepTime'], errors='coerce')
            input_df['CRSArrTime'] = pd.to_numeric(input_df['CRSArrTime'], errors='coerce')
            
            # Calculate flight duration
            input_df['FlightDuration'] = input_df['CRSArrTime'] - input_df['CRSDepTime']
            
            # Add distance (you might want to add a distance lookup)
            # For now, we'll use a placeholder
            input_df['Distance'] = 500  # Placeholder distance
            
            # Add weather features if not provided
            weather_features = ['temperature_c', 'precip_mm', 'cloud_pct', 'wind_speed_mps']
            for feature in weather_features:
                if feature not in input_df.columns:
                    input_df[feature] = 0  # Default values
            
            # Add delay features (set to 0 for prediction)
            delay_features = ['LateAircraftDelay', 'WeatherDelay', 'CarrierDelay', 'NASDelay', 'SecurityDelay']
            ...
            if self.preprocessor is not None:
                ...
                confidence = min(0.95, max(0.60, self.results.get('Best_Model', {}).get('r2', 0.74)))
                
                return {
                    'predicted_delay': round(prediction, 2),
                    'confidence': round(confidence, 3),
                    'model_used': 'United Airlines Enhanced Predictor',
                    'status': 'success'
                }
            else:
                raise ValueError("Preprocessor not available. Please train the model first.")
